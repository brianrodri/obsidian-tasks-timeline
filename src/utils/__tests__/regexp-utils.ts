...
describe("splitAtRegExp", () => {
    it("returns array with one string if pattern not found", () => {
        ...
        expect(splitAtRegExp(text, regExp)).toEqual([text]);
    });
    ...
    it("returns array with interspersed matches", () => {
        ...
        expect(splitAtRegExp(text, regExp)).toEqual(["def", ":::", "a", ":", "ghi", "::", "bc", ":", "jkl"]);
    });

    it("asserts input is string", () => {
        expect(() => splitAtRegExp(3 as unknown as string, new RegExp(/^.*$/g))).toThrow();
    });

    it("asserts input is RegExp", () => {
        expect(() => splitAtRegExp("5", 3 as unknown as RegExp)).toThrow();
    });
});
